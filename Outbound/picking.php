...
if (isset($_SESSION['wms']['wmsDefComp']))
    $comp = $_SESSION['wms']['wmsDefComp'];
else
    $comp = 1;
if (isset($_SESSION['wms']['zones']))
    $zones = $_SESSION['wms']['zones'];
else
    $zones = array();
$zones = array();

if (isset($_REQUEST['noSelect']))
    $noSelect = $_REQUEST['noSelect'];
else
    $noSelect = 0;
...
$i = 0;
$stat = "-1|3";
if ($orderType <> "%" and $orderType <> "O")
    $stat = "{$orderType}|{$orderType}";
if ($orderType == "%")
    $stat = "-1|3";
if ($orderType == "O")
    $stat = "4|9";
    ...
    foreach ($rowData as $rec => $l) {
        ...
        if (1 == 1) {
            ...
            if (count($d) > 0 and !$addLink) {
                foreach ($d as $pd) {
                    if ($pd['user_id'] == $UserId and $l['order_stat'] < 3)
                        $addLink = true;
                }
...
HTML;
    if ($other == "" and $notrel == "" and $needprod == "")
        $h5 = "";
