...
if ($ponum < 1) { // no PO#, error, redirect back to POstatus.php
    $htm = <<<HTML
<!DOCTYPE html>
<html lang="en">
<head>
...
$batchPrompt = "";
if ($batch <> "") $batchPrompt = "<br> Batch # {$batch}";
$backOnclick = "'" . $Redirect . "?nh=" . $nh . "';";
$htm = <<<HTML
      ...
      <table class="table table-bordered table-striped overflow-auto">
       <thead>
        <tr>
         ...
         <th class="FieldCaptionTD">Recvd so Far</th>
         <th class="FieldCaptionTD">Stocked</th>
         <th class="FieldCaptionTD">Tote/Bin</th>
        ...
        <tr v-for="det in detailData">
         ...
         <td align="right" v-bind:class="setClass(det.qty_recvd,det.qty_ord)">
{{ det.qty_recvd }}
<button class="btnlink" @click="updQty(det);" value="Add"><img border="0" src="../images/edit2.png" title="Edit Record"></button>
</td>
         <td align="right" v-bind:class="setClass1(det.qty_recvd,det.qtyStocked)">{{ det.qtyStocked }}</td>
         <td align="left">{{ det.pack_id }}</td>
         <td>{{ det.line_type }}</td>
        </tr>
       </table>
     </div>
       ...
       <div class="modal-dialog">
        <div class="modal-content">
         <div class="modal-header">
          <button type="button" class="close" @click="editAU=false"><span aria-hidden="true">&times;</span></button>
<span>editAU content</span>
         </div>
        </div>
       </div>
      </div>
...
var application = new Vue({
 ...
 alert(JSON.stringify(det));
  ...
  setClass1(qty_rec,qty_stocked)
  {
   var rcls="";
   if (qty_stocked < qty_rec) { rcls="Alt5DataTD"; }    
   if (qty_stocked > qty_rec) { rcls="Alt7DataTD"; }
   return(rcls);
  },
