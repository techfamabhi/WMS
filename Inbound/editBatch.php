...
if (isset($func) and $func == "adjRecpt") {
    ...
    */
    ...
    $k = 0;
    $stockd = "";
    if ($data["rcvAdj"] <> 0 or $data["stkAdj"] <> 0) { // adjusting recvd or stockd
...
SQL;
    ...
    if ($data["recv_to"] == "a" and $opt[27] < 1) {
        if (isset($data["Bins"][$data["Tote"]]) and isset($data["toteDtl"])) { // adjust tote or bin
            $tote = $data["Tote"];
            $tt = $data["Bins"][$tote];
            $code = "";
...
else { // load stockvw and tote info
...
if (isset($result["WhseLoc"]) and count($result["WhseLoc"]) > 0) {
    if (count($result["WhseLoc"]) > 1) { // reorder result so totes are on top
        $na = array();
        $nb = array();
        $cont = true;
