...
if ($func == "") { // display Aisles
    $lastBin = "";
    $aCnt = countAisles($db, $comp);
    $dhtm = frmtAisles($aCnt);
} // display Aisles

if ($func == "goToAisle") { // go to first part in aisle
...
if ($func == "scanUPC" and $idx > 0) {  // got the part, scan the UPC
    $part = $rec[$idx];
...
HTML;
    if (count($aisles) < 1) { // there are no aisles to display
        $htm .= <<<HTML
           <tr>
