...
$pg->jsb = <<<HTML
<script>
if (typeof scaninput !== 'undefined') { document.form1.scaninput.focus(); }
    ...
    <div class="w3-half">
      <div class="w3-container w3-blue w3-padding-8">
        <div class="w3-clear"></div>
                <label>Vendor</label>
<select class="w3-white" onchange="do_bin();" name="scaninput" placeholder="Select Vendor">
...
HTML;
    ...
    foreach ($po as $key => $row) {
        ...
        $htm .= <<<HTML
       <tr>
        <td><input type="checkbox" name="POs[]" value="{$row["wms_po_num"]}" onclick="chk_sel();"></td>
...
HTML;
    ...
    if ($scaninput <> "" and $j) { // its numeric must be a upc
        $upc = $scaninput;
        if ($upc == $shd) $upc = ".{$shd}";
...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8">
<link href="../Themes/Multipads/Style.css" type="text/css" rel="stylesheet">
HTML;
