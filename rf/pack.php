...
// pack.php -- Packing
// 04/04/22 dse initial
// 01/03/24 dse change Pack it to Finalize
/*TODO

Check if lines are unpicked and somehow determine if we should be allowed
to Pack it and ship it. (in packit switch)

problems

Currently updating the ORDTOTE table to PACK when the user scans a tote.
Add input field to current order screen for more totes if preset
Once all totes are scanned, allow them to place it in a container
containers are a tote with a type of "TMP"


Other Problems
If a part is overpulled, what do you do about it?
 prompt user to Remove it from the order, 
 then remove it from Item pull, 
 but then it needs to go somewhere to be put back
 away on the shelf so it can be re-picked when ordered

Also, it could be a supercede, in which case, the user would have to tell 
the screen which line to replace.
...
switch ($func) {
    ...
    case "packit":
    {
        ...
        if (isset($w["Order"])) { // display Order and Tote Info
...
//print_r($w); 
    ...
    case "orderOrTote":
    {
        if (isset($scaninput) and $scaninput <> "") { // get info for scanned order or scanned tote
            $req = array("action" => "fetchOrder",
                "company" => $comp,
                "scaninput" => $scaninput,
                "process" => "PACK"
...
//print_r($w);
    ...
    case "scanVerify":
    {
        ...
        if (isset($scaninput) and trim($scaninput) <> "") { // scaninput is set
            if (!isset($msg)) $msg = "";
            $req = array(
                "action" => "scanVerify",
                "company" => $comp,
            ...
            if (isset($w["errText"])) { // there is a message
                $msg = $w["errText"];
            } // there is a message
        } // scaninput is set
        if (isset($scanTote)) $tmp_totes = $scanTote;
        if (isset($totes) and is_array($totes)) { // there is more than 1 tote to verify
            $comma = "";
            $tmp_totes = "";
...
//echo "</pre>";
    ...
    case "scanVerDone":
    {
        ...
        $ordInfo = "";
        if (isset($ord["Order"])) { // display Order and unset tote and line item info
            unset($ord["LineTote"]);
            unset($ord["Totes"]);
            unset($ord["unPicked"]);
            unset($ord["Items"]);
...
HTML;
            ...
            $i = 0;
            if (isset($t["contents"])) {
                if (count($t["contents"]) > 9) { // too many parts to show on small screen, link to detail
                    $i = count($t["contents"]);
                    $addShowDetail = true;
                    $conthtm .= <<<HTML
          <tr>
...
HTML;
                ...
                else { // less than 10 parts, show on screen
        ...
        $ret = <<<HTML
         <table class="table table-bordered table-striped">
          ...
          <tr>
           ...
           <th class="FieldCaptionTD">2 Ship</th>
           <th class="FieldCaptionTD">UOM</th>
