...
//exit;
        ...
        if (isset($_REQUEST['func']) and $_REQUEST['func'] == "putBin") { // reset back to scan the part again
...
if ($func == "palletToMove" and $toteId <> "") { // press Random Putawat for the user right now
    $func = "whatToDo";
    $R1 = 1;
...
//echo "</pre>";
                    ...
                    if (isset($part['choose'])) { // find which part is in the tote
...
//Should be PARTSRV, but the update fails
        ...
        if (isset($ok) and $ok) { // its ok, update it
            ...
            if (($opt[27] > 0 and $toteType == "RCS") or $toteType == "MOV" or $toteType == "RET") { // part has already been stocked, move it to the bin
...
//exit;
            ...
            else { // move part to bin and update stocked and inventory
                if (is_array($wmspo) and  ($wmspo > 0)) {
                    $ok = true;
                    $qtyLeft = $Qty;
...
HTML;
    ...
    if ($tQty > $Qty) {
        ...
        $msg2 = <<<HTML
        <span class="Alt7DataTD" 
        style = "word-wrap: normal;font-weight: bold; font-size: large; margin-left: 0px; text-align: center;" >
            Total $tQty of this Part are in this Tote 
        </span >
        <br > 
       <p>Scan the Bin to put this item into</p> 
HTML;
    }
    $oc = "validateBin(this);";
...
//echo " </pre > ";
    $save_sendToBin = base64_encode(json_encode($data));
    $data['hiddens'] .= <<<HTML
  <input type="hidden" name="save_sendToBin" value="{
            $save_sendToBin}">
