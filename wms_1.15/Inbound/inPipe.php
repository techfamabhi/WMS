...
exit;
       ...
       if (!isset($Vendors[$vendor]) and $vendor <> "")
        { // vendor not found
	 $output[]= "Row {$rowcnt} Invalid Vendor {$vendor}\n";
         $inValidVend++;
        } // vendor not found
       if (isset($Vendors[$vendor]) or $vendor == "")
        { // vendor found
        //Update, Insert or Delete the record
         $where=<<<SQL
where pl_code = "{$pl}"
...
exit;
 ...
 // check if P/L and part number is set, if not error out
       if (isset($rowData["shadow_number"]) and $rowData["shadow_number"] < 1)
...
exit;
	 ...
	 $rowData["part_class"]=$part["Part"]["part_class"];
         ...
         if (trim($rowData["whse_loc"]) == "" or $rowData["qty_in_primary"] == 0)
     ...
     case "DRP":
      ...
      { // order num is not empty
       ...
       { // valid order
        ...
        $rowData["order_num"]=$Order;
        $comp=$validOrd["company"];
        if (isset($newCust) and $newCust == true and trim($newCustNum) <> "")
        { // update customer record with DRP record name and address
