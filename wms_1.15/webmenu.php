...
require("config.php");
if (!isset($wmsInclude)) $wmsInclude=getcwd() . "/include";
require_once("{$wmsInclude}/chklogin.php");
//$rc=chk_login();
$username="";
$mtype=0;
// Set local vars from session
foreach (array_keys($_SESSION["wms"]) as $w) { $$w=$_SESSION["wms"][$w]; }
/* Session vars
UserID
UserLogin
GroupID
...
if (isset($first_name) and isset($last_name)) $username="{$first_name} {$last_name}";
if ($username <> "") $pg->User["Name"]=$username;
$pg->menuScript=$myself;
$pg->Display();
if (isset($_REQUEST["menu_num"])) $menu_num=$_REQUEST["menu_num"]; else $menu_num = $menu_number;
