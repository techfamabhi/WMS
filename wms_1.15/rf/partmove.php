...
// if part is set, check if part is in this bin, if not, throw a error.
     if (isset($args["shadow"]) and $args["shadow"] > 0)
     { // set source or dest bin
      if (!isset($args["sourceBin"]))
      {
       if (isset($args["WhseLoc"]) and count($args["WhseLoc"]) > 0)
...
HTML;
   ...
   if (isset($result["Part"]) and $result["Part"]["shadow_number"] > 0 and isset($result["WhseLoc"]) and (isset($args["sourceBin"]) and $args["sourceBin"] <> ""))
   { // check if part is in this bin
 ...
 if ($numParts == 1)
  { // display part info
   $shadow=$result["Part"]["shadow_number"];
   if (isset($args["shadow"]) and $args["shadow"] <> $shadow)
...
if (isset($result) and isset($result["infoType"]) and $result["infoType"] == "P" and $result["numRows"] > 1)
{ // format choose
  $msgcolor="yellow";
    $title=<<<HTML
...
$qm=0;
if (isset($args["sourceBin"]) and $args["sourceBin"] <> "" and isset($args["WhseLoc"]))
...
function isPartinBin($shadow, $WhseLoc, $sourceBin)
{
 ...
 $ok=false;
 if ($shadow > 0 and is_array($WhseLoc) and count($WhseLoc) > 0 and $sourceBin <> "")
   { // check if part is in this bin
