...
HTML;
 ...
 if ($ord == null)
 { // done picking the orders, load the orders anyway and set finc to complete
...
{
   if ($myZones1 == "") $z="";
   else $z=" and whse_loc like [{$myZones1}]%";
...
order by whse_loc,shadow

SQL;

   $tmp=$db->gData($SQL);
   if (isset($tmp[1])) $next_line=$tmp[1];
   else $next_line=array();
...
//function pickComplete($thisprogram,$nh,$hostOrder,$order_num, $dropZone="")
 ...
 $orders=array();
echo "<pre>";
 if (is_array($ord) and count($ord) > 0)
 { // ord is array
 foreach ($ord as $key=>$o)
  {
   if (is_numeric($key))
...
exit;
 ...
 if (isset($toteId) and intval($toteId) > 0 and $dropZone <> "")
 {
  ...
  $rdata=relOrder($comp, $order_num, $myZones,$dropZone);
  if (isset($ret["status"]))
   {
    $stat=$ret["status"];
    $msg=$ret["msg"];
...
function relOrder($comp, $order_num, $myZones,$dropZone)
{
 global $RESTSRV;
 global $W2ErpSRV;
