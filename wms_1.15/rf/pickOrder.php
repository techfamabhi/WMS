...
/*TODO
problems
 ) Tote # is not verified if re-entered because of invalid or in use tote causing # 2 problem
2) If tote is 0 or not valid, parts don't get added to tote, causing 
   order to be stuck in status 3, so order won't finalize
 ) Drop zone is not verified not allowing order to be finalized 

Older ones
 ) Totes and Drops
   Need to program multiple tote ids for an order
   need to verify the tote when multiple tote are used
   need place to store all drop locations for a tote (last loc in ORDTOTE)

 ) need to add button for new tote when an orders tote is full
 ) need to update ITEMPULL and add tote id so packing can verify contents
...
$msgcolor="";
$js="";
if (isset($func) and $func == "pickScanPart" and isset($NoUPC) and $NoUPC == "1")
...
if ($B1=="NFP" and $func == "pickScanPart")
{ // zero pick the item
...
if ($func == "pickGoToBin" and $B1=="NFP")
{ // check if any items have been picked, it not free the tote
 ...
 //echo "<pre>At check if any items have been picked, it not free the tote";
      if (isset($response["zeroed"]) and $response["zeroed"] > 0)
...
//exit;
      ...
      else if (isset($response["picked"]) and $response["picked"] < 1)
...
switch ($func)
{
 case "enterOrd":
 {
  ...
  "host_order_num"=>$scaninput
   ...
   { // order found
    ...
    $mainSection=letsPick($ord,"Pick It");
 ...
 case "enterTote":
 {
  ...
  if ($toteId <> "")
  {
   ...
   if (isset($tc["numRows"]) and $tc["numRows"] < 1)
   { // invalid tote
...
*/
       ...
       and isset($line1[1]["zero_picked"]))
       {
        $found=false;
        foreach($_SESSION["wms"]["Pick"] as $jj=>$pk)
        {
           //echo " zero={$pk["zeroed"]} ?= {$line1[1]["zero_picked"]}\n";
         if ($pk["line_num"] == $line1[1]["line_num"])
          {
...
//echo "<pre> cur_loc={$cur_loc} Next_loc={$next_loc}";
//print_r($next_line1);
//print_r($_REQUEST);
//echo "</pre>";
//exit;
...
//exit;
 ...
 case "Drop":
 {
   ...
   $msg="Order {$hostordernum} Complete";
   $mainSection=pickComplete($thisprogram,$nh, $hostordernum, $orderNumber, $scaninput);
   break;
 } // end Drop
} // end switch func
...
function checkDrop($opt,$thisprogram,$nh,$orderNumber,$hostordernum,$msg="")
{
 global $toteId;
...
HTML;
  ...
  { // redirect for next pick
   $ret["msg"]="Completed Picking for {$hostordernum}";
   $ret["mainSection"]=pickComplete($thisprogram,$nh,$hostordernum,$orderNumber);
  } // redirect for next pick
 return $ret ;
} // end checkDrop
...
//Need to loop thru all totes for the order
 ...
 // or free the totes, then Ship with  WMS2ERP.php server
  ...
  fclose($fp);
   if (isset($rdata["openLines"]) and $rdata["openLines"] < 1 and $opt[25] > 0)
   { // all lines picked, ship it
    if ($rdata["openLines"] == 0)
...
function do_reset()
{
 document.location.href="pickQue.php?nh={$nh}";
}
</script>
</body>
</html>
...
function chkIfMore($orderNumber,$orderNumber,$zones,$hostordernum,$toteId,$cur_loc)
{
 global $thisprogram;
 global $nh;
 global $RESTSRV;
...
function displayItems($items,$allowSkip=0)
{
...
HTML;
 ...
 {
  $oc="";
  if ($allowSkip == 1) $oc=" onclick=\"skipToLine({$i["line_num"]});\"";
  if ($allowSkip == 2) $oc=" onclick=\"skipToLine(0);\"";
  $btn=<<<HTML
<button class="binbutton-tinyer" title="Pick this Item" name="Bs" onclick="{$i["line_num"]}">Pick{$ext}</button>
HTML;
 if ($allowSkip == 2) $btn=<<<HTML
...
HTML;
   $htm.=<<<HTML
        <tr{$selected}{$oc}>
         ...
         <td align="center">{$i["qtytopick"]}</td>
