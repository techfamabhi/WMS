...
if (isset($_REQUEST["startrec"])) $startrec=$_REQUEST["startrec"]; else $startrec=1;
$numrecs_deflt=false;
if (isset($_REQUEST["numrecs"])) $numrecs=$_REQUEST["numrecs"]; 
else 
{
 $numrecs=25;
 $numrecs_deflt=true;
}
if (isset($_REQUEST["comp"])) $comp=$_REQUEST["comp"]; else $comp=0;
if (isset($_REQUEST["esrc"])) $esrc=$_REQUEST["esrc"]; else $esrc="";
if (isset($_REQUEST["shadow"])) $shadow=$_REQUEST["shadow"]; else $shadow="NULL";
if (isset($_REQUEST["partnum"])) $partnum=$_REQUEST["partnum"]; else $partnum="";
if (isset($_REQUEST["epl"])) $epl=$_REQUEST["epl"]; else $epl="";
if (isset($_REQUEST["epo"])) $epo=$_REQUEST["epo"]; else $epo="";
if (isset($_REQUEST["eref"])) $eref=$_REQUEST["eref"]; else $eref="";
if (isset($_REQUEST["ept"])) $ept=$_REQUEST["ept"]; else $ept="";
if (isset($_REQUEST["ebin"])) $ebin=$_REQUEST["ebin"]; else $ebin="";

$comp_select=company_dropdown($db,$comp,"All");
if (isset($esrc) and $esrc <> "") $first_field="eref"; else $first_field="ebin";
...
$ept=strtoupper($ept);
if ($esrc=="") $src="%"; else $src=$esrc;
if ($ebin=="") $bin=""; else $bin=$ebin;

if ($ept=="%") $ept="";
if ($ept=="") $ptype="%"; else { $ptype="[{$ept}]"; } 

//$startrec=13;
if ($epl=="") $pl="%"; else $pl=$epl;
if ($epo=="") $po="%"; else $po=$epo;
if ($eref=="") $ref="%"; else $ref=$eref;
$req=array(
"action"=>"fetchAll",
"company"=>$fcomp,
"startRec"=>$startrec,
...
if ($ebin <> "%" and $ebin <> "") $ok=true;
if ($src <> "%" and $src <> "") $ok=true;
if ($shadow <> "NULL") $ok=true;
if ($ok)
{
//echo "<pre>{$SQL}\n</pre>";
//exit;
...
if (isset($svData["rowData"]) and $svData["rowData"] > 0) $svarray=$svData["rowData"];
//echo "<pre>";
//print_r($svData);
//exit;
   ...
   $db->next_record();
     if ($numrows and $db->Record)
     {
      foreach ($db->Record as $key=>$data)
       {
        if (!is_numeric($key)) { $svarray[$i]["$key"]=$data; }
       }
     }
    $i++;
  } // while i < numrows
...
$epta[3]="";
$epta[4]="";
if ($ept == "%")       $epta[0]=" selected";
if ($ept == "")       $epta[0]=" selected";
if ($ept == "ICT")  $epta[1]=" selected";
if ($ept == "PRDBA") $epta[2]=" selected";
if ($ept == "LX")      $epta[3]=" selected";
if ($ept == "C")      $epta[4]=" selected";
...
if (!isset($oldpartnum)) $oldpartnum="";
$htm=<<<HTML
...
HTML;
if (count($svarray) > 0) foreach ($svarray as $row=>$data)
{
$htm_sv.=<<<HTML
 <tr>
...
HTML;
   ...
   $cls="";
   if ($t == "C") $cls="Alt2DataTD";
   if ($t == "D") $cls="AltDataTD";
   if ($t == "A") $cls="AltDataTD";
   if ($t == "L") $cls="Alt3DataTD";
   if ($t == "T") $cls="Alt6DataTD";
   if ($t == "PIC") $cls="Alt5DataTD";
   if ($t == "RCV") $cls="Alt4DataTD";
   if ($t == "B") $cls="Alt4DataTD";
   $tdTitle="";
