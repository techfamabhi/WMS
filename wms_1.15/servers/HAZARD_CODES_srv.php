...
// HAZARD_CODES_srv.php -- Server for HAZARD_CODES.php
//12/10/21 dse initial
//02/09/22 dse move field defs to function in srv_hdr.php


$update_table="HAZARD_CODES";
$query_table="HAZARD_CODES";
$DEBUG=true;
require("srv_hdr.php");
...
if ($DEBUG) wr_log("/tmp/HAZARD.log","inputData={$inputdata}");
$action=$reqdata["action"];
if (isset($reqdata["haz_code"])) $haz_code=$reqdata["haz_code"]; else $haz_code="";
...
if ($DEBUG) wr_log("/tmp/HAZARD.log","Switching={$action}");
switch ($action)
{
 case "fetchall":
 case "fetchSingle":
 {
  $where="";
  $order_by="order by haz_code";
  if ($haz_code <> "") $where=<<<SQL
where haz_code = "{$haz_code}"
...
SQL;
  if ($DEBUG) wr_log("/tmp/HAZARD.log","SQL=" . $SQL);
  ...
  if (isset($x)) unset($x);
  $x=json_encode($rdata);
  if ($DEBUG) wr_log("/tmp/HAZARD.log",$SQL);
  if ($DEBUG) wr_log("/tmp/HAZARD.log",$x);
  echo $x;
   break;
 } // end fetchs
...
SQL;
  ...
  if ($DEBUG) wr_log("/tmp/HAZARD.log","In Update Mode SQL=");
  if ($DEBUG) wr_log("/tmp/HAZARD.log",$SQL);
  $currec=array();
  $rc=$db->query($SQL);
  $numrows=$db->num_rows();
  if ($DEBUG) wr_log("/tmp/HAZARD.log","{$numrows} records");
  $i=0;
  while ($i <= $numrows)
  {
   $db->next_record();
  ...
  if ($DEBUG) wr_log("/tmp/HAZARD.log","{$numrows} records");
  $j=count($currec);
 if ($DEBUG) wr_log("/tmp/HAZARD.log","found {$j} records");
 if ($j > 0)
...
SQL;
  ...
  if ($found_diff > 0)
  {
   ...
   if ($DEBUG) wr_log("/tmp/HAZARD.log",$msg);
   $rdata='{"message":"' . $msg . '"}';
  }
  else
  {
   if ($DEBUG) wr_log("/tmp/HAZARD.log","No fields to Update");
...
SQL;
  ...
  if ($Cnt > 0)
  {
   $msg="Hazard Code {$haz_code} is Already is use, Record not Added.";
   if ($DEBUG) wr_log("/tmp/HAZARD.log",$msg);
   $rdata='{"message":"' . $msg . '"}';
   echo $rdata;
   break;
  ...
  if ($DEBUG) wr_log("/tmp/HAZARD.log","upd_flds={$upd_flds}");
  if ($DEBUG) wr_log("/tmp/HAZARD.log","updVals={$updVals}");
  $SQL=<<<SQL
