...
*/
 ...
 if ($ord->Order["order_stat"] == 4)
 { // stat = 4, set ship qty, adjust alloc, book parthist and set status to 5
...
*/
  ...
  $qty_core=0;
  $qty_def=0;
  if (intval($inv_code) == 1)
  { // core
   $qty_core=$item["qty_ship"];
   $qty=0;
  }  // core
  if (intval($inv_code) == 2)
  { // defect
...
//exit;
   $xl=array();
   if (count($ord->Items)) foreach($ord->Items as $l=>$ii) {$xl[$ii["line_num"]]=$l;}
   
   foreach ($ord->ItemPull as $key=>$item)
   {
    if (isset($item["line_num"]))
    { // item is set
    $l=$xl[$item["line_num"]];
