...
$i=1;
 ...
 unset($db);
  $ok=true;
  if (!isset($auth["apiKey"])) $ok=false;
  if (isset($auth["apiKey"]) and $auth["apiKey"] == "") $ok=false;
  if (!isset($auth["partnerId"])) $ok=false;
  if (isset($auth["partnerId"]) and $auth["partnerId"] == "") $ok=false;
  if (!isset($auth["systemId"])) $ok=false;
  if (isset($auth["systemId"]) and $auth["systemId"] == "") $ok=false;
 ...
 public function Send($type, $post,$errId=0) 
 {
  ...
  if ($type == "Ship") $url="{$this->url}/orderShipped";
  if ($type == "Receive") $url="{$this->url}/orderReceived";
  if ($type == "Adjust") $url="{$this->url}/inventoryAdjustment";
  if ($type == "Error") $url="{$this->url}/errorNotification";
  $auth=$this->auth;
...
SQL;
    ...
    { // Add New wms Comm Error
     ...
     $SQL=<<<SQL
insert into WMSCOMMERR
(utcDate, recordType, statusCode, retryTimes, lastRetry, payload, response)
...
SQL;
 ...
 private function getToken()
 {
  ...
  $auth=$this->auth;
  if (is_array($auth)) $post=json_encode($auth);
                       else return false;
  $ch = curl_init($turl);
  $options=array(
   CURLOPT_RETURNTRANSFER => true,
