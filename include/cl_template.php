 ...
 * 03/01/2022 Dave Erlenbach (DSE) Change the setting of the template filename
 *                 allowing programer to control the exact path of template
 * 03/01/2022 DSE  Add public path() function
 *                 Goal was to allow path or theme to set language and theme
...
//DSE Change template filename to eliminate extsrainious dots and slashes

                //$this->config->file = $this->_path(TRUE) . $this->config->theme . '/' .  $this->config->template_suffix . '/' . $input . '.' . $this->config->extension;
                $fileName = "";
                $fileName = $this->aFileName($fileName, $this->_path(TRUE));
                $fileName = $this->aFileName($fileName, $this->config->theme);
                $fileName = $this->aFileName($fileName, $this->config->template_suffix);
                $fileName = $this->aFileName($fileName, $input);
                $fileName = $this->aFileName($fileName, $this->config->extension, ".");
...
// end DSE
     ...
     * aFileName
     * @access private
     * @param file, string file name we are building
     * @param add, string to add to the file
     * @param type, string to separate the stying we are adding to the file
     * @return string
     * @usage aFileName($file,"123");
     *  or    aFileName($file,"123",".");
     *  or    aFileName($file,"123","");
     *
     * Add a string to build the filename
     * with path, theme, prefix, file, suffix
     * */
    private function aFileName($file, $add, $typ = "/")
    {
        if (empty($file)) $file = "";
        if ($file == "" and $typ == "/") $typ = "";
        if (!empty($add) and trim($add) <> "") $file .= "{$typ}{$add}";
        return $file;
    }
    ...
    private function _conditions()
    {
        ...
        if ($options > 0) {
            ...
            foreach ($options[0] as $i => $option) {
                ...
                $statement = $this->_statement = $options[0][$i];
                $_type = $options[1][$i];
    ...
    function _try_parse($code)
    {
        ...
        $rand = sha1(microtime()); // Creates watchpoint
    ...
    private function _variables()
    {
        ...
        if ($this->config->clean) {
            ...
            $this->data->input = preg_replace_callback('#{nocode:(.+?)}#', function ($m) {
