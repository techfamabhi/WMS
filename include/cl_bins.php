...
class BIN
{
    ...
    var $binQty;
    var $db;
    var $tmp = array(); // end function init
...
//echo "<pre>{$SQL}\n";
        ...
        if ($numRows > 0) { // found 1 or more parts, get other locations for the part
            if ($numRows > 1) { // loop thru all parts and set otherBins
                foreach ($ret as $key => $r) {
                    $w = $this->getOtherBins($r);
                    if (count($w) > 0) $ret[$key]["otherBins"] = $w;
                } // end foreach ret
            } // loop thru all parts and set otherBins
            else { // get other bins for the 1 record
...
//echo "<pre>{$SQL}\n";
    ...
    function updWhseLoc($comp, $loc, $shadow, $code, $qty, $uom)
    {
        ...
        $old_qty = 0;
        if ($whsLoc["numRows"] < 1) { //insert new record
            $SQL[$nextSQL] = <<<SQL
insert into WHSELOC
...
SQL;
        ...
        if ($whsQty["numRows"] < 1) { //insert new Qty record
            $SQL[$nextSQL] = <<<SQL
insert into WHSEQTY
...
SQL;
        ...
        else { // update existing Qty record
            $extra = "";
