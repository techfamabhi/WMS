...
class Report
{
    ...
    function Run()
    {
        if (isset($this->B1)) { // begin Run Report Section
...
//exit;
        ...
        else { // begin report selection criteria
    ...
    function format_thdr($hdr, $fields)
    {
        if ($this->excel or $this->pipe) {
            ...
            foreach ($hdr as $key => $d) {
                if (substr($fields[$key], 0, 1) <> "H") { // not hidden field
                    if ($this->pipe) {
                        $htm .= <<<TEXT
{$key}|
TEXT;
...
HTML;
                ...
                case "popup":  // checkbox field
                    ...
                    $this->popup = 1;
                    $j = strpos($href, "{");
                    if ($j > 0) { // search for variables to replace
