...
class WMS_DB
{
    ...
    function rmquotes($Query_String)
    {
        ...
        $SQL = $Query_String;
        if ($this->replace_quotes and strpos($SQL, '"')) { // there are quotes , may have to escape single quotes if present
            $SQL = str_replace('"', "'", $SQL);
        } // there are quotes
        return $SQL;
    } // end queryAll
    ...
    function metadata($table)
    {
        ...
        */
        ...
        foreach ($this->Link_ID->query("show columns from $table") as $row) {
            $res[$i]["table"] = $table;
            foreach ($row as $key => $data) {
                if (in_array($key, $reqflds) and !is_numeric($key)) {
                    $res[$i]["$key"] = $data;
                }
            }
            $numrows = $i;
    ...
    function Trans($typ)
    {
        ...
        switch ($typ) {
            case "B":
                if ($this->DBDBG <> "")
                    $this->debugLog("Begin Transaction");
                $this->Link_ID->beginTransaction();
                break;
            case "C":
                if ($this->DBDBG <> "")
                    $this->debugLog("Commit Transaction");
                $this->Link_ID->commit();
                break;
            case "R":
                if ($this->DBDBG <> "")
                    $this->debugLog("Rollback Transaction");
