...
if (isset($_REQUEST["searcH"]))
    ...
    $srch = "";
if ($DEBUG)
    wr_log("/tmp/PICK.log", "inputData={$inputdata}");
$action = $reqdata["action"];

if ($action == "")
    if (isset($reqdata["scaninput"]))
        $scaninput = $reqdata["scaninput"];
    else
        $scaninput = "";
if (isset($reqdata["company"]))
    $comp = $reqdata["company"];
else
    $comp = 1;
if (isset($reqdata["case"]))
    $case = $reqdata["case"];
else
    $case = "";
if (isset($reqdata["custname"]))
    $custname = $reqdata["custname"];
else
    $custname = "";
if (isset($reqdata["order_num"]))
    $order_num = $reqdata["order_num"];
else
    $order_num = "";
if (isset($reqdata["process"]))
    $process = $reqdata["process"];
else
    $process = "";
if (isset($reqdata["line_num"]))
    $line_num = $reqdata["line_num"];
else
    $line_num = "";
if (isset($reqdata["partNumber"]))
    $partNumber = $reqdata["partNumber"];
else
    $partNumber = "";
if (isset($reqdata["updqty"]))
    $updqty = $reqdata["updqty"];
else
    $updqty = 0;
if (isset($reqdata["pull_num"]))
    $pull_num = $reqdata["pull_num"];
else
    $pull_num = "";
if (isset($reqdata["user_id"]))
    ...
    $user_id = 0;
if (isset($reqdata["shadow"]))
    $shadow = $reqdata["shadow"];
else
    $shadow = "";
if (isset($reqdata["qtyPicked"]))
    $qtyPicked = $reqdata["qtyPicked"];
else
    $qtyPicked = "";
if (isset($reqdata["uom"]))
    $uom = $reqdata["uom"];
else
    $uom = "";
if (isset($reqdata["p_l"]))
    $p_l = $reqdata["p_l"];
else
    $p_l = "";
if (isset($reqdata["part_number"]))
    $part_number = $reqdata["part_number"];
else
    $part_number = "";
if (isset($reqdata["picked"]))
    $picked = $reqdata["picked"];
else
    $picked = "";
if (isset($reqdata["pickuom"]))
    $pickuom = $reqdata["pickuom"];
else
    $pickuom = "";
if (isset($reqdata["host_order_num"]))
    $host_order_num = $reqdata["host_order_num"];
else
    $host_order_num = "";
if (isset($reqdata["order_type"]))
    $order_type = $reqdata["order_type"];
else
    $order_type = "";
if (isset($reqdata["skipLines"]))
    $skipLines = $reqdata["skipLines"];
if (isset($reqdata["priority"]))
    $priority = $reqdata["priority"];
else
    $priority = "";
if (isset($reqdata["ship_via"]))
    $ship_via = $reqdata["ship_via"];
else
    $ship_via = "";
if (isset($reqdata["order_stat"]))
    $order_stat = $reqdata["order_stat"];
else
    $order_stat = "";
if (isset($reqdata["sortby"]))
    $sortby = $reqdata["sortby"];
else
    $sortby = array();
if (isset($reqdata["statRange"]))
    $statRange = $reqdata["statRange"];
else
    $statRange = "-1|1";
if (isset($reqdata["zone"]))
    $zone = $reqdata["zone"];
else
    $zone = "";
if (isset($reqdata["zones"]))
    $zones = $reqdata["zones"];
else
    $zones = array();
if (isset($reqdata["tote_id"]))
    $tote_code = $reqdata["tote_id"];
else if (isset($reqdata["tote_code"]))
    $tote_code = $reqdata["tote_code"];
else
    $tote_code = "";
if (isset($reqdata["totes"]))
    $totes = $reqdata["totes"];
else
    $totes = "";
if (isset($reqdata["whseLoc"]))
    $whseLoc = $reqdata["whseLoc"];
else
    $whseLoc = "";
if (isset($reqdata["origBin"]))
    $origBin = $reqdata["origBin"];
else
    $origBin = "";
if (isset($reqdata["newBin"]))
    $newBin = $reqdata["newBin"];
else
    $newBin = "";
if (isset($reqdata["qty"]))
    $qty = $reqdata["qty"];
else
    $qty = "";

if (isset($reqdata["numRows"]))
    $numRows = $reqdata["numRows"];
else
    $numRows = 10;
if (isset($reqdata["startRec"]))
    $startRec = $reqdata["startRec"];
else
    $startRec = 0;
if ($srch <> "")
    $comp = intval($srch);
...
if ($DEBUG)
    wr_log("/tmp/PICK.log", "Switching={$action}");
$rdata = array();
switch ($action) {
    case "fetchOrder": // fetch open orders for packing
    {
        $lookupBy = 0;
        $awhere = "";
        if ($scaninput <> "") { // check either host order# or tote id scanned
            // check host order#
            $SQL = "select order_num from ORDERS where company = {$comp} and host_order_num = '{$scaninput}'";
            ...
            if ($order_num < 1) { // check by tote
                $SQL = "select order_num from ORDTOTE where tote_id = {$tote_id}";
            ...
            $lookByOrder = true;
            if ($order_num <> "")
                $awhere .= " and order_num = '{$order_num}' ";

            if ($host_order_num <> "")
                $awhere .= " and host_order_num = '{$host_order_num}' ";
            ...
            if ($lookByOrder) {
                $SQL = "select order_num from ORDERS where company = {$comp} {$awhere}";
            } // end lookByOrder
            else {
                $SQL = "select order_num from ORDTOTE {$awhere} ";
            } // look by tote
        ...
        if ($order_num > 0) {
            ...
            $ret["LineTote"] = array();
            if ($lookupBy == 2 and $scaninput <> "" and $process == "PACK") { // packing scanned this tote, set location to PACK
...
SQL;
    ...
    case "fetchAll": {
        $awhere = "";
        if ($custname <> "")
            $awhere .= "and CUSTOMERS.name like \"{$custname}%\"\n";
        if ($order_num <> "")
            $awhere .= "and order_num = \"{$order_num}\"\n";
        if ($host_order_num <> "")
            $awhere .= "and host_order_num = \"{$host_order_num}\"\n";
        if ($order_type <> "")
            $awhere .= "and order_type = \"{$order_type}\"\n";
        if ($priority <> "")
            $awhere .= "and priority = {$priority}\n";
        if ($ship_via <> "")
            $awhere .= "and ship_via = \"{$ship_via}\"\n";
        if ($order_stat <> "")
            $awhere .= "and order_stat = {$order_stat}\n";
        $ob = 0;
        if ($statRange <> "" and $action <> "fetchOrder") {
            $w = explode("|", $statRange);
            if ($w[0] == 4 and $w[1] == 9)
                $ob = 3;
            if ($w[0] == 1 and $w[1] == 1) { // looking for open picks
                $ob = 1;
                $awhere .= " and ( select count(*) from ITEMPULL where ord_num = ORDERS.order_num and qty_picked < qtytopick) > 0 ";
            } // looking for open picks
            else {
                $awhere .= "and (order_stat between {$w[0]} and {$w[1]}";
                if ((2 >= $w[0]) && (2 <= $w[1]))
                    $awhere .= " or order_stat = -2\n)";
                else
                    $awhere .= ")\n";
            }
        }
        ...
        $SQL = "select count(*) as cnt from ORDERS  where company = {$comp} {$awhere} ";
        ...
        if ($ob > 0)
            $order_by = "order by order_stat desc, wms_date,order_num";
        //if ($ob > 2) $order_by="order by order_stat desc,pic_done desc, wms_date,order_num";
        if ($ob > 2)
            $order_by = "order by wms_complete desc,order_stat desc,pic_done desc, wms_date,order_num";
...
SQL;
                    ...
                    if (
                        strpos($rdata["rowData"][$rowsReturned]["zones"], ",")
...
SQL;
        ...
        $x = json_encode($rdata);
        if ($DEBUG)
            wr_log("/tmp/PICK.log", "COMP:{$comp} startRec={$startRec} numRows={$numRows} rowsReturned={$rowsReturned}");
        if ($DEBUG)
            wr_log("/tmp/PICK.log", $SQL);
        if ($DEBUG)
            wr_log("/tmp/PICK.log", $x);
...
SQL;
    ...
    case "fetchPickOrder":  // get the open lines for a pick [within a zone]
    {
        ...
        if ($line_num <> "")
            $l = intval($line_num);
        if ($pull_num <> "")
            $pull = intval($pull_num);
        $z = "";
        //if ($zone <> "") $z=$zone;
        $rdata = getPick($db, $o, $l, $zones, $pull, $user_id, 0);
        // get zero picked items too
        if (isset($rdata["errCode"]) or count($rdata) < 1)
            $rdata = getPick($db, $o, $l, $zones, $pull, $user_id, 1);
        if (isset($x))
            unset($x);
        $x = json_encode($rdata);
        if ($DEBUG)
    ...
    case "flagOrder": // show all zones on a order
    {
        ...
        if ($pull_num <> "")
            $pull = intval($pull_num);
        if ($picked <> "")
            $pick .= ",qty_picked = qty_picked + {$picked}";
        if ($pickuom <> "")
            $pick .= ",uom_picked = \"{$pickuom}\"";
...
SQL;
                ...
                if ($DEBUG)
                    wr_log("/tmp/PICK.log", $w);
                if ($DEBUG)
                    wr_log("/tmp/PICK.log", "numRows: {$db->NumRows}");
                echo $w;
                break;
            } //all good to update
            else { // problem with tote status
    ...
    case "updToteLoc": {
        ...
        $tote_id = getToteId($tote_code);
        if ($DEBUG)
            wr_log("/tmp/PICK.log", "in UpdOrdTote: ord={$order_num} tote={$tote_id} loc={$whseLoc}");
        if ($order_num > 0 and $tote_id > 0 and $whseLoc <> "")
            $rc = updOrdTote($db, $order_num, $tote_id, $zone, $whseLoc);
        if ($DEBUG)
            wr_log("/tmp/PICK.log", "UpdOrdTote: rc={$rc}");
        echo "{rc:{$rc}}";
        break;
    } // end updOrdTote
    ...
    case "updToteLoc2": {
        $tote_id = getToteId($tote_code);
        if ($DEBUG)
            wr_log("/tmp/PICK.log", "in UpdOrdTote2: ord={$order_num} tote={$tote_id} loc={$whseLoc}");
        if ($order_num > 0 and $tote_id > 0 and $whseLoc <> "")
            $rc = updOrdTote2($db, $order_num, $tote_id, $zone, $whseLoc);
        if ($DEBUG)
            wr_log("/tmp/PICK.log", "UpdOrdTote2: rc={$rc}");
        echo "{rc:{$rc}}";
        break;
    } // end updOrdTote2
...
*/
    ...
    case "scanVerifyTote": // scan verify tote contents for packing
    {
        ...
        if (trim($partNumber) <> "") {
            ...
            if ($rdata["status"] == 1) { // got 1 part
                $shadow = $rdata["Part"]["shadow_number"];
            } // end got 1 part
            else if ($rdata["status"] > 1) { // do choose
                $ret = $rdata;
            } // status > 1 do choose
        } // end partNUmber <> ""
        else
    ...
    case "scanVerify": // Fscan verify order contents for packing
    {
        ...
        if (trim($partNumber) <> "") {
            ...
            if ($rdata["status"] == 1) { // got 1 part
                ...
                if ($tqty < 0)
                    $updqty = abs($tqty);
                if ($tqty < 0 and $updqty > 0)
                    $updqty = $updqty * abs($tqty);
                if ($updqty == 0)
                    $updqty = 1;
    ...
    case "verifyResults": // scan verify order contents for packing
    {
        ...
        if (count($items) > 0 and count($scanned) > 0) {
            foreach ($items as $key => $item) {
                ...
                $rdata[$l]["status"] = 0;
                if (intval($item["qty_ord"]) - intval($sc) > 0)
                    $s = -1;
                if (intval($item["qty_ord"]) - intval($sc) < 0)
                    $s = 1;
                $rdata[$l]["status"] = $s;
            } // end foreach items
            if (count($scanned) > 0) { // add not on order parts to results
...
function getItems1($db, $order, $line = 0, $sortby = 0)
{
    ...
    $orderby = "";
    if ($sortby == 1)
        $orderby = "order by whse_loc , A.ord_num, line_num";
...
function getPick($db, $order, $line = 0, $zones = array(), $pull = 0, $user_id = 0, $zeros = 0)
{
    ...
    if ($line > 0)
        $awhere .= " and A.line_num = {$line}\n";
    if ($pull > 0)
        $awhere .= " and A.pull_num = {$pull}\n";
    if ($zeros > 0) { // search for zero picked regardless of user
        $awhere .= " and A.zero_picked > 0\n";
    } // search for zero picked regardless of user
    else { // only search for items that have not been zero picked
        $awhere .= <<<SQL
 and A.zero_picked = 0
-- and (user_id = 0 or user_id = {$user_id})
...
SQL;
    ...
    if ($DEBUG)
        wr_log("/tmp/PICK.log", "Zones: " . json_encode($zones));
    ...
    if (isset($skipLines) and is_array($skipLines) and count($skipLines) > 0) { // its a array of lines
...
SQL;
    ...
    if (!isset($zone))
        $zone = "";
    if (!empty($zones) and count($ret) < 1)
        return array("errCode" => -35, "errText" => "No Records for Zone:{$zone}");
    else
        return $ret;
} // end getPick
...
SQL;
    $ret = $db->gData($SQL);
    if (count($ret) < 1) { // no open order found, lets see if it's valid and get status
        $SQL = <<<SQL
...
SQL;
        ...
        if (count($ret1) < 1)
            ...
            return array("errCode" => 1, "errText" => "Order: {$order} has no items in this zone or is complete");
...
function getOrderToPick($db, $host_order, $user_id = 0)
{
    ...
    global $DEBUG;
    $extraWhere = "";
    if ($case <> "" and $case <> "showAll")
        $extraWhere = "and order_stat < 3";
    else
        $extraWhere = <<<SQL
and ((user_id = 0 or user_id = {$user_id})
...
SQL;
        ...
        if (count($ret1) < 1)
            ...
            return array("errCode" => 1, "errText" => "Order: {$host_order} has no items in this Zone or is complete");
    } // no open order found, lets see if it's valid and get status
    else {
...
SQL;
        ...
        if ($db->NumRows > 1) { //consolidate the order and sum up zones
            $zones = "";
...
SQL;
    ...
    $rtype = 0;
    if ($updqty <> 0 and $numrows > 0) { // update 1st record I find
        $ctrlQty = $updqty;
...
function scanOrd($db, $order, $shadow, $qty, $uom, $user = 0)
{
    ...
    $scanLine = 0;
    $msg = "";
    if (checkPartOnOrder($db, $order, $shadow) < 1)
        $msg = "The Part is not on this Order";
    $SQL = <<<SQL
select
scan_line,
qty_scan
...
SQL;
    ...
    if ($scanLine > 0) { // query was succesful, update the row
        $SQL = <<<SQL
 update PACKSCAN
...
SQL;
    ...
    else { // check if valid part for this order and add new scan record
        $SQL = <<<SQL
...
SQL;
        ...
        else { // line not found, add new scan record
...
SQL;
    ...
    $ret = array("rc" => $rc);
    if ($rcode == -1)
        $ret["errText"] = "The Part is not on this Order";
    if ($msg <> "")
        $ret["errText"] = $msg;
    $ret["totQty"] = $totqty;
    return $ret;
...
function countPickedItems($db, $order_num, $lines)
{
    ...
    if (is_array($lines) and count($lines) > 0) {
        foreach ($lines as $lineNum => $line) {
            ...
            if (!isset($Items[$lineNum]["qty2Pick"]))
                $Items[$lineNum]["qty2Pick"] = 0;
            if (!isset($Items[$lineNum]["qtyPicked"]))
                $Items[$lineNum]["qtyPicked"] = 0;
...
SQL;
            ...
            if ($picked > $toPick)
                $cls = "Alt5DataTD"; // blue
            if ($picked == $toPick)
                $cls = "Alt4DataTD"; // green
            if ($toPick == 0 and $picked == 0)
                $cls = "Alt3DataTD"; // gray, no inventory
