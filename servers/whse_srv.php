...
SQL;
    ...
    case "movePart":
    {
        ...
        */
        ...
        if ($ok == 0) { // all good, we can move it
            //Load part
            $pr = new PARTS;
            $pr->Load($shadow, $comp);
            $part = array();
...
//Loop this to get source and dest 
                ...
                if (substr($theBin, 0, 1) == "!") { // its a tote
            ...
            if ($fromTo["to"]["type"] == "T") { // set tote type to move and update last location
...
//print_r($part);
    ...
    case "findIt":
    {
        ...
        if (trim($Search) <> "") {
            ...
            if ($numParts == 1) {
                $shadow_number = 0;
                if (isset($rdata["Result"]["shadow_number"])) { // we found a part
