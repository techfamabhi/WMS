...
SQL;
        ...
        if (!isset($rdata[1])) { //part not found in this tote
            $rdata["errCode"] = -1;
...
SQL;
            ...
            $rc = $db->query($SQL);
            $numrows = $db->num_rows();
            if ($numrows > 0) { // update tote location
                if ($zone <> "" and $newBin <> "") {
                    $SQL = <<<SQL
update RCPT_TOTE
set last_zone = "{$zone}",
...
SQL;
        ...
        $rdata = $db->gData($SQL);
        $numRows = $db->NumRows;
        if ($numRows < 1) { // try if it is a bin
            $bincls = new BIN;
...
SQL;
    ...
    case "addTask": // Add new Task
    {
        ...
        $rdata = array();
        $tote_num = getToteId($tote_id, $comp);
        if ($tote_num > 0 and $task_type <> "" and $userId > 0) { // got minimum data, proceed
            ...
            if (count($check) < 1 or $task_id < 1) { // get new task#
...
SQL;
    ...
    {
        $goodAction = true;
        foreach (array_keys($reqdata) as $w) {
            $$w = $reqdata[$w];
        }
        /*
        Args in
...
if ($goodAction) { // return data not empty
    $x = json_encode($rdata);
...
SQL;
    ...
    if ($ret["numRows"] > 0) foreach ($ret as $rec => $d) {
        if (trim($d["primary_bin"]) == "") { // get prodline pref zone/aisle
            $SQL = <<<SQL
