...
$db1->DBDBG = "/tmp/alloc2.log";
$logfile = "/tmp/itempull.log";
$logfile1 = "/tmp/itempullu.log";
/* TODO
...
while ($i <= $numrows) { // while i < numrows
    $db->next_record();
    if ($numrows and $db->Record) { // numrows and db rec
        foreach ($db->Record as $key => $data) { // foreach db rec
            if (!is_numeric($key)) {
                $ords[$i]["$key"] = $data;
            }
        } // foreach db rec
...
if (count($ords) > 0) { //count ords > 0
    foreach ($ords as $orders) { // foreach ords as orders
        $updOrder = false;
...
SQL;
        ...
        if ($order_stat == -35) { // stat = -35
            echo "Order Not Found";
            $SQL = <<<SQL
            
SQL;
            $rc = $db2->Update('delete from ORDQUE where order_num = {$ord}');
        } // stat = -35
        if ($ship_complete == "Y") { // ship complete = Y
...
SQL;
        ...
        if (isset($items[1])) { // isset items[1]
            foreach ($items as $rec => $item) { // foreach items
                $shipSoFar = 0;
                $process = 0;
                $process = 0;
                $qtyord = $item["qty_ord"];
                ...
                if (!is_null($item["qty_avail"])) { // start avail > qty to ship
                    $process = 1;
                    $shadow = $item["shadow"];
                    $SQL = <<<SQL
 select 
...
SQL;
                    ...
                    $ovstqty = 0;
                    if (count($bins) > 0)
                        foreach ($bins as $key => $bin) { // foreach bins
                            if ($bin["whs_qty"] > 0) {
                                if ($bin["whs_code"] == "P") { //get primary bin info
                                    foreach ($bin as $key1 => $b) { // foreach bin
                                        $f = $key1;
                    ...
                    if (!isset($primary_qty)) $primary_qty = 0;
                    if ($leftToFill > 0 and $primary_qty >= $leftToFill) { // fill from primary
                    ...
                    if ($leftToFill > 0) { // not enough in primary, lets try primary and overstock
                        $totQty = 0;
                        if ($orderedBins["primQty"] > 0) { // pull as much as possible from primary bin
                            if ($debug) echo "primary qty={$orderedBins["primQty"]}\n";
                            if ($orderedBins["primQty"] >= $leftToFill) $fill = $leftToFill;
                            else $fill = $orderedBins["primQty"];
                            $leftToFill = $leftToFill - $fill;
                        ...
                        if ($leftToFill > 0 and $orderedBins["ovstQty"] > 0) { // loop thru overstock to pull rest
                            //loop 1, check to see if 1 overstock bin has enough
                            foreach ($orderedBins["O"] as $ovnum => $o) { // loop 1
                                if ($leftToFill > 0 and $o["whs_qty"] > 0) { // found it, fill it and end
                                ...
                                else { // fill any avail and keep going
                                    if ($leftToFill < $o["whs_qty"]) { // there is enough to fill the rest in this bin
                                        $fill = $leftToFill;
                                    } // end there is enough to fill the rest in this bin
                                    else { // not enough, fill what we can
                                        $fill = $o["whs_qty"];
                    ...
                    if ($leftToFill > 0 and $ship_complete <> "Y") { // lets ship partial
...
//exit;
                ...
                { // isset itempull[1]
                    ...
                    $SQL = array();
                    foreach ($itempull as $k => $ip) { // foreach itempull
...
SQL;
                        $i = 0;
                        foreach ($ip as $fld => $val) { // foreach ip
                            if ($fld <> "qty_avail") {
                                $c = ",";
                                if ($i == 0) $c = "";
                                $q = "";
...
SQL;
                        if ($ip["qtytopick"] == 0) {
                            wr_log($logfile1, "Unsetting: {$SQL[$k]}");
                            wr_log($logfile1, json_encode($ip));
                            unset($SQL[$k]);
                        } else {
                            wr_log($logfile, $SQL[$k]);
                            wr_log($logfile, json_encode($ip));
                        }
...
SQL;
                if (isset($SQL) and is_array($SQL) and count($SQL) > 0) foreach ($SQL as $k => $usql) { // foreach SQL
                    // if ($debug) echo "{$usql}\n";
                    wr_log($logfile, $usql);
                    $rc = $db1->Update($usql);
                    wr_log($logfile, "Update returned={$rc}");
                    echo "SQL# {$k}, rc={$rc}\n";
                    if ($rc <> 1) {
                        wr_log($logfile1, $usql);
                        wr_log($logfile1, "Update returned={$rc}");
                    }
                } // end foreach SQL
