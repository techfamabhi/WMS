<?php
//proc_upc.php -- process uploaded bins
// 03/06/23 dse adapted from proc_bins.php
/* TODO
 1) default alt_type_code to -1 if not set
 2) add Delete if action = "D"
*/
...
if (($handle = fopen($fullname, "r")) !== FALSE) {
    while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
        ...
        if ($row > 1) {
            ...
            $alts[$row]["pl"] = onlyascii($data[0]);
            $alts[$row]["partnum"] = onlyascii($data[1]);
            $alts[$row]["alt_part_number"] = onlyascii($data[2]);
            $alts[$row]["alt_shadow_num"] = 0;
            // Set Qty
            if (!isset($data[3])) $data[3] = 1;
            if (!is_numeric($data[3])) $data[3] = 1;
            $alts[$row]["alt_type_code"] = onlyascii($data[3]);
            if ($alts[$row]["alt_type_code"] == 0) $alts[$row]["alt_type_code"] = 1;
            $alts[$row]["alt_type_code"] = -$alts[$row]["alt_type_code"];
            //Set UOM
            if (!isset($data[4])) $data[4] = "EA";
            $alts[$row]["alt_uom"] = onlyascii($data[4]);
            $alts[$row]["alt_sort"] = 0;

            $pl = trim($alts[$row]["pl"]);
            $part = trim($alts[$row]["partnum"]);
            $upc = trim($alts[$row]["alt_part_number"]);
            $uSQL = "";
            $shadow = get_shadow($db, $pl, $part);
            $alts[$row]["alt_shadow_num"] = $shadow;
            $altRec = array();
            if ($shadow < 1) {
                $nofpart++;
                $nofbins .= <<<HTML
...
HTML;
            ...
            if ($shadow > 0) {
                unset($alts[$row]["pl"]);
                unset($alts[$row]["partnum"]);
                $altRec = get_alt($db, $shadow, $upc);
                ...
                if (isset($alts[$row]["alt_type_code"])
                    and $alts[$row]["alt_type_code"] > 0) { // set type code to - type code and add if not o file
                    $alts[$row]["alt_type_code"] = -$alts[$row]["alt_type_code"];
                    $rc = chkAltType($db, $alts[$row]["alt_type_code"]);
                } // set type code to - type code and add if not o file

                if (count($altRec) < 1) { // alt not found add it
                    $rec_add++;
                    $comma = "";
                    $dataset = "";
                    $ifields = "alt_part_number,alt_shadow_num,alt_type_code,alt_uom,alt_sort";
                    ...
                    $uSQL = <<<SQL
insert into ALTERNAT ({$ifields})
values ( {$dataset} );

SQL;
                } // alt not found add it
                else { // alt is there, update it
                    $updRec = true;
                    ...
                    if ($updRec) {
                        ...
                        foreach ($alts[$row] as $key => $Fld) {
                            if ($key <> "alt_shadow_num" and $key <> "alt_part_number") {
                                ...
                                $val = trim($Fld);
                                if (isset($altRec[$key]) and $val <> trim($altRec[$key])) { // field is changed
                                    if ($types[$key] < 1) $val = '"' . trim($Fld) . '"';
                                    $dataset .= "{$comma}\n{$set} {$key}={$val}";
                                } // field is changed
                            } // not shadow or upc
